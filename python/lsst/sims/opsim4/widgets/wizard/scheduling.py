...
class SchedulingPage(QtWidgets.QWizardPage):
    ...
    def __init__(self, parent=None, is_general=True):
        ...
        label12 = QtWidgets.QLabel("Set the hour angle maximum for ranking requested fields.")
        label12.setWordWrap(True)

        hour_angle_max_la = QtWidgets.QLabel("Hour Angle Max:")
        hour_angle_max_le = QtWidgets.QLineEdit("6.0")
        hour_angle_max_la.setBuddy(hour_angle_max_le)
        hour_angle_max_le.setValidator(QtGui.QDoubleValidator(0.1, 12.0, 1))
        hour_angle_max_un = QtWidgets.QLabel("hours")
        self.registerField("{}_scheduling_hour_angle_max".format(self.param_tag), hour_angle_max_le)
        ...
        if self.is_general:
            ...
            scroll_area_widget_layout.addWidget(restrict_grouped_visits_cb, 13, 1)
            scroll_area_widget_layout.addWidget(label_img, 14, 0, 1, 3)
            scroll_area_widget_layout.addWidget(label6, 15, 0, 1, 3)
            scroll_area_widget_layout.addWidget(time_interval_la, 16, 0)
            scroll_area_widget_layout.addWidget(time_interval_le, 16, 1)
            scroll_area_widget_layout.addWidget(time_interval_un, 16, 2)
            scroll_area_widget_layout.addWidget(label7, 17, 0, 1, 3)
            scroll_area_widget_layout.addWidget(time_window_start_la, 18, 0)
            scroll_area_widget_layout.addWidget(time_window_start_le, 18, 1)
            scroll_area_widget_layout.addWidget(label8, 19, 0, 1, 3)
            scroll_area_widget_layout.addWidget(time_window_max_la, 20, 0)
            scroll_area_widget_layout.addWidget(time_window_max_le, 20, 1)
            scroll_area_widget_layout.addWidget(label9, 21, 0, 1, 3)
            scroll_area_widget_layout.addWidget(time_window_end_la, 22, 0)
            scroll_area_widget_layout.addWidget(time_window_end_le, 22, 1)
            scroll_area_widget_layout.addWidget(label10, 23, 0, 1, 3)
            scroll_area_widget_layout.addWidget(time_weight_la, 24, 0)
            scroll_area_widget_layout.addWidget(time_weight_le, 24, 1)
